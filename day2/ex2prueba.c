...
static int check_aux(int *report, int len)
{
    int i = 0;
    int asc_flag = 0;
...
static int check_report(int *report, int len, int problem_dampener)
{
    int i = 0;
    int asc_flag = 0;
    if ((report[i + 1] - report[i] > 0 && report[i + 2] - report[i + 1] > 0)
    ...
    while (i < len - 1)
    {
        ...
        if (!((diff > 0 && diff < 4 && asc_flag) || (diff < 0 && diff > -4 && !asc_flag)))
        {
            if (problem_dampener)
            {
                int *aux_report1 = malloc((len - 1) * 4);
                int *aux_report2 = malloc((len - 1) * 4);
                int j = 0, k = 0;
                while (j < len - 1)
                {
                    if (k == i)
                        k++;
                    aux_report1[j] = report[k];
                    j++;
                    k++;
                }
                j = 0, k = 0;
                while (j < len - 1)
                {
                    if (k == i + 1)
                        k++;
                    aux_report2[j] = report[k];
                    j++;
                    k++;
                }
                int value = check_aux(aux_report1, len - 1) + check_aux(aux_report2, len - 1);
...
int main()
{
    ...
    while (i < 1000)
    {
        ...
        safe_reports += check_report(report, len, 1);
        free(report);
    }
